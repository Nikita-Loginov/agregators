...
const createMarkerIcon = (warehouse) =>
  L.divIcon({
    html: `
      <div class="${CONFIG.MARKER.CLASS} ${
      warehouse.active ? CONFIG.MARKER.ACTIVE_CLASS : ""
    }">
...
export const initSimpleMap = (idMap = "map", warehouses = [], options = {}) => {
  ...
  const setActiveMarker = (marker) => {
    ...
    if (activeMarker) {
      activeMarker.__warehouse.active = false;
      activeMarker.setIcon(createMarkerIcon(activeMarker.__warehouse));
    }

    marker.__warehouse.active = true;
    marker.setIcon(createMarkerIcon(marker.__warehouse));

    activeMarker = marker;

    map.setView(marker.getLatLng(), CONFIG.MAP.ACTIVE_ZOOM, { animate: true });
  };
  ...
  const markers = warehouses.map((warehouse) => {
    ...
    marker.on("click touchend", () => setActiveMarker(marker));
